...
## Project structure
- `skills/<name>/SKILL.md` — **source of truth**, one dir per skill (`whoami`, `entry`, `writing`, `design`, `weaving`, `inquiry`, `assembly`, `integrity`, `intake`, `on-duty`, `methodology-work`, `verstakify`, `product-roadmap`); `references/*` optional (`whoami`, `verstakify`, `writing`, `product-roadmap` ship them).
