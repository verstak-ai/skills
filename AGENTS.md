...
- **Agent karta**: `#931 «👨‍💻 Разработчик скилл-репозиториев агента»` — adhikarin, steward of #844 (also stewards the iskron/skills fork #1506). Your inbox: `nks_orient(realm="nks-dev", focus="931")` at session start (self-locate fallback: `nks_me(action="kartas")`).
...
## Project structure
- `skills/<name>/SKILL.md` — **source of truth**, one dir per skill (`memory`, `entry`, `writing`, `design`, `weaving`, `inquiry`, `assembly`, `integrity`, `intake`, `on-duty`, `methodology-work`, `verstakify`, `product-roadmap`); `references/*` optional (`memory`, `verstakify`, `writing`, `product-roadmap` ship them).
