...
## Project structure
- `skills/<name>/SKILL.md` — **source of truth**, one dir per skill (`memory`, `entry`, `writing`, `design`, `weaving`, `inquiry`, `assembly`, `integrity`, `reality-audit`, `intake`, `on-duty`, `methodology-work`, `verstakify`, `product-roadmap`); `references/*` optional (`memory`, `verstakify`, `writing`, `product-roadmap` ship them).
