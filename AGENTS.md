...
## Project structure
- `skills/<name>/SKILL.md` — **source of truth**, one dir per skill (`minding`, `entry`, `writing`, `design`, `weaving`, `inquiry`, `assembly`, `integrity`, `reality-audit`, `intake`, `on-duty`, `collaborate`, `methodology-work`, `verstakify`, `product-roadmap`); `references/*` optional (`minding`, `verstakify`, `writing`, `product-roadmap` ship them).
