...
- `.claude-plugin/plugin.json` — the plugin manifest. Its `version` (semver) is bumped automatically by [release-please](https://github.com/googleapis/release-please) from the Conventional Commit history, and that bump is what makes `/plugin marketplace update` deliver the new version to installs.
