...
| **connect** | Reach the graph when the harness cannot: decide native-vs-bridge by a ladder of checkable signs, raise the shipped `nks-bridge` (stdio ↔ streamable-HTTP with the full OAuth flow, never answers with silence), first-aid for half-dead `mcp-remote`. |
