...
This is the move agents get wrong. **`source_kind=shabda` is provenance** (this entered as external word) — orthogonal to **epistemic mode** (how well we know it). `kalpita` is *not* a blanket stamp; it is the mode of an **unverified claim**. Per methodology #104:
...
- **`kalpita` is never permanent.** It is the entry state of the шабда→сверка→тезис thread (#340); step 4 graduates it.

Model the node as methodology #104 prescribes: `attrs.source_kind="shabda"`, the given_as the content warrants, the epistemic mode from the table.
...
Intake is not done when the claim is written — it's done when it's been *reconciled with reality* (methodology #157, the сверка kriya). For each kalpita node, observe directly on the three levels (#157): `nks_look` (recall — what's written), `nks_orient(lens="tensions")`/neighborhood (awareness — what pulls), `nks_orient(lens="trace")` (reflection — where it travels). Then graduate:

- **claim matches reality** → assert the thesis (#155): `nks_update` the epistemic mode up (kalpita → pratyakshita/pramanita), or let it stand as confirmed;
- **claim contradicts reality** → that's a tension, not a fact: leave it `badhita`, or raise a `vyabhichara`/`samshaya` (#157 routes сверка-found counter-examples to vyabhichara, not samshaya), or fix the incident;
- **mode merely drifted** → update the recording mode (#156).

A realm that ingests but never verifies is exactly the #1158 failure with extra steps.

## 6. Selectivity is a guard, not a convenience

Never the whole tracker, never "all the docs". Declare the cut (open/labelled, a date bound, the high-signal subset), cap the volume, and **state what you dropped** — a silent cap reads as "ingested everything". Selectivity + the shabda modes + dedup are together the mitigation of #1158; drop any one and the guard fails.
...
| Question | Node |
|---|---|
| Shabda-entities and their modes (the #104 table) | `nks_look(node_id="104", realm="methodology")` |
| Впуск шабды — the intake kriya | `nks_look(node_id="165", realm="methodology")` |
| Сверка шабды пратьякшей | `nks_look(node_id="157", realm="methodology")` |
| Нить 4 «Внешнее слово» (шабда→сверка→тезис) | `nks_look(node_id="340", realm="methodology")` |
| Утверждение тезиса / update of recording mode | `nks_look(node_id="155", realm="methodology")` · `#156` |
