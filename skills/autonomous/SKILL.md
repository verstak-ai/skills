...
5. **Stand on the socket** with whatever your harness watches sockets with — in Claude Code that is `Monitor` with a `ws` source and `persistent: true`, which watches read-only, so **speaking back needs its own route**: a doer holds exactly one socket and a second connection displaces the first, so publish and send over the addresses handed to you beside it rather than opening a writer. **Reconnect whenever it closes** — nothing promises to keep it alive, so reopening is the ordinary course, not an incident, and a watch gone deaf is indistinguishable from a quiet inbox. A drop costs the waking, never the words: what arrived waits in the queue. Idleness closes the channel itself, and `connect` raises it again — except on a `leaving` close, where the seat is still yours and you reattach with the same token. Other named closes and the per-harness detail are in **collaborate**'s `references/channel.md`.
