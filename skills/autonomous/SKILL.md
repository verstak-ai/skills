...
**A channel is two capabilities, and only one of them gets tested by accident.** Hearing is proven the first time anything arrives. Speaking is proven by nothing at all — a published socket, an armed hook and a green connect describe a doer that can be reached, and say nothing about whether it can reach anyone. Steps 1–3 below are all receiving; step 5 is the one that closes the loop, and it exists because a watch that hears and cannot answer looks, from the inside, exactly like a watch that is working.
...
5. **Prove you can speak — send the opening delta to your human.** One real message, not a ping: where you stand, what you are holding, what you expect to be asking them about. Take the address from `nks_channel(action="list")` — the standing that bridges to that person, read off the board rather than guessed, and never substituted by the `me` marker, which names the karta bound to *your* identity and has nothing to do with their bridge. **If it does not go through, the watch has not begun**: repair the channel — that *is* the tact — instead of going off to work behind a mouth that doesn't open.

**Sending is the cheapest live probe of the surface you stand on.** A limit you reason your way to costs a session; a limit you find by trying to speak costs one call, and it comes back as a fact rather than a belief. This is how surface defects actually surface — not by reading the tool's own description of itself, but by attempting the move.
...
- **After the merge**: run the repo's post-merge ritual, weave the wake (§4), and where the merge moved ground a neighbour stands on, say so in *their* inbox as a delta. Then the branch is deleted and the scratch is dropped — yours to clean, in the same tact.
...
- **Hearing is half a channel.** The outgoing path is proven by sending, never by connecting — and a watch that cannot reach its human has not begun.
- **Sending is the cheapest probe of the surface you stand on**, and a blocker you have not probed is a guess.
...
## What autonomous is NOT

- Not **assembly** (the realm-wide pass) and not bare **inquiry** — it composes inquiry per item and adds the repo work, the relay, the integration, and the wait.
- Not **collaborate** — that is one exchange with one doer; this is the cycle that keeps making them.
...
- Not unsupervised: autonomy is over the *span* of the work, never over the will that sanctions it. Everything in §5 still goes up.
