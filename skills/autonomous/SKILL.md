...
1. **Look before you mint.** `nks_channel(action="list")` — a doer holds at most one live channel, and a second mint answers 409. Yours already there and the socket address still in hand? Stand on it. Lost the socket, or the TTL closed it? `revoke`, then mint again.
2. **Mint.** `nks_channel(action="mint", karta=<your karta>)` returns two addresses that are not the same kind of thing: the **inbound** `https://…` is a mailbox address — other doers write to it, and it is meant to be findable; the **socket** `wss://…` is shown **once** and is the credential itself. Whoever holds the socket reads everything addressed to you.
...
4. **Stand on the socket** with whatever your harness watches sockets with, and **reconnect when it closes** — the socket is not durable, and a watch gone deaf is indistinguishable from a quiet inbox. Idleness closes the channel by TTL (default 6h), so a long silence needs a re-mint, not a reconnect.
