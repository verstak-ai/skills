...
- **The doer becomes a steward.** Create the repo's agent karta
  ...
  (**memory** §4); without it their roles stay unlinked per realm. Record both
...
- Local project-memory dir (`~/.claude/projects/<encoded-path>/memory/`):
  ...
  (**memory**); agent working-style preferences → the user's global harness
