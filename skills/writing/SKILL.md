...
5. **Do not confuse graph checks with reality evidence.** `CHECKS:`, trace connectivity, actors, and legal arrows establish graph integrity only. For a behavioral `sachverhalt`, code inspection is `anumita`; observing a focused internal/mock-only test can justify `pratyakshita`; `pramanita` requires the separate **reality-audit** skill: observable contract, canonical public boundary, attempted falsifier, and fresh independently observable evidence. Any executable evidence used to mint `pratyakshita` or `pramanita` must have a recorded successful exit status. Green lines inside an aggregate command that exits nonzero do not qualify. A failing broad suite does not automatically refute a separately isolated fact, but without a focused exit-zero falsifier for that exact fact, keep it `anumita` and attach a reverify vimarsha instead of upgrading it. A newly written black-box test can qualify when it rebuilds and executes that exact boundary; authorship alone neither upgrades nor downgrades it. Mint behavioral `pramanita` only after the audit verdicts are frozen and no material artifact change remains in the phase — never between probes. A later patch invalidates that confidence until affected public evidence is rerun and the node reverified. If the evidence surface is unavailable, keep the claim provisional and anchor a reverify vimarsha instead of stamping confidence.
