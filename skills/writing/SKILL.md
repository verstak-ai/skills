...
5. **Do not confuse graph checks with reality evidence.** `CHECKS:`, trace connectivity, actors, and legal arrows establish graph integrity only. For a behavioral `sachverhalt`, code inspection is `anumita`; observing a focused/self-authored test can justify `pratyakshita`; `pramanita` requires the separate **reality-audit** skill: observable contract, public boundary, attempted falsifier, and fresh independent evidence. If the evidence surface is unavailable, keep the claim provisional and anchor a reverify vimarsha instead of stamping confidence.
