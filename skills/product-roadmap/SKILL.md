...
description: "Use this skill to build a product roadmap for a GitHub repository — or a multi-repo product (a whole org, or a set of repos that together form one product) — derived from its issues and PRs through an NKS reasoning graph. Triggers: 'roadmap from repo', 'build a roadmap from issues', 'roadmap from github', 'roadmap for my org', 'multi-repo roadmap', 'roadmap across repos', 'quick roadmap', 'roadmap teaser', 'собери роадмап из issues/PR', 'роадмап по продукту', 'что в этом репо делать дальше', product-roadmap. Role-plays the maintainer: models the current product as a verified present-state ground, intakes the weighted backlog as shabda, assembles the directions underway (bianhua), and renders a figure-on-ground roadmap as graph + markdown + local HTML. Composes verstakify (codebase understanding), intake (shabda discipline), and assembly (direction discernment). Needs the gh CLI and nks_* MCP tools."
---
