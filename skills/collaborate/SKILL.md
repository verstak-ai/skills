...
**Being audible is not being able to speak, and only one of the two is ever tested by accident.** Everything above is the receiving half: it is proven the first moment anything arrives. The sending half is proven by nothing — a connect that returned, a hook that armed and an address others can see all describe a doer that can be *reached*. So test it deliberately, with one real message that had to be sent anyway rather than a ping, and test it early, because a doer who discovers at the end of a session that its mouth never opened has lost the session, not a call.

**Sending is also the cheapest live probe of the surface you are standing on.** A limit you deduce costs hours and comes back as a belief; a limit you find by attempting the move costs one call and comes back as a fact — and where the surface's own description disagrees with what the attempt returns, the attempt wins and the gap goes to whoever owns that surface. This is the ordinary way defects in a channel get found: not by reading about it, but by trying to talk.
...
- **Listening is half of collaborating.** A doer who sends but never opened its own socket is not in the exchange, only looks like it — to the other side and to itself. **Speaking is the other half, and it is proven by sending, never by connecting** — accepted is not delivered, and a path counts as proven when a word comes back along it.
...
- **Muting the siblings on your own role** — declining the events another standing of the same karta caused, while everything from outside the role still reaches you. The platform now carries the handle and the client half is written and waiting to merge, so this silence is **temporary rather than permanent** — once it deploys, muting becomes teachable and gets its own pass. Until then the only instrument you actually hold is saying what you are busy with, so a neighbour can route around you rather than wait. The entry stays here so the gap is *stated* rather than passed over: a move described but not deployed reads exactly like a move described correctly, and that costs more than the silence.
