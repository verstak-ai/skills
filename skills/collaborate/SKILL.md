...
**Get the socket — `connect`.** `nks_channel(action="connect", karta=<your own>)` converges to a working socket from any starting state, so you need not know your own history first: no channel, it opens one; a live channel, it reissues the socket alone and leaves the inbound address, the queue and the trail untouched. It hands back that inbound address — which you give out and register as your hook target — and the socket, **shown once and never listed again**. `mint` only opens what isn't there, so its 409 on a live channel is correct behaviour, not an obstacle. And **`revoke` is never the way back**: it destroys the inbound address others already hold and takes with it whatever pointed at it, including your karta's own subscription to its own inbox — which is exactly how a doer returns looking connected and deaf. Revoke answers a leaked address, nothing else.

**Then hold it — this is the step that gets skipped.** An unattached socket is a published address nobody answers at, and both consequences are silent: messages sit undelivered, and the idle window (counted from socket activity, not from arrivals) closes the channel out from under you. So hand it to whatever your harness watches sockets with, and keep it for as long as you might be answered; the first frame on the way in tells you how much waited. In Claude Code a read-only watch is `Monitor` with a `ws` source and `persistent: true`, and each frame then lands as a notification inside your turn. But a built-in watcher is usually receive-only, while a doer has exactly one socket — a second connection displaces the first — so speaking back needs a route of its own. Look first for a write path that needs no writing socket: where the platform offers one, its address comes back from the same call that handed you the socket, and an ordinary POST is enough. Failing that, one connection does both — a small script that prints each frame on stdout for the harness to surface, sends what you hand it, and reopens on close. Before concluding your harness cannot send at all, look for the plain websocket client already inside it (Node 22 and later expose `WebSocket` globally). "There is no way" is a claim about your own toolbox first.

What you send up is your occupation. A channel list shows who is connected; only a doer's own line separates one at work from one merely attached. Nobody can write it for you — a watcher's account of what you are doing is a guess wearing an observation's clothes.

**Then check, rather than assume.** `nks_channel(action="list")` shows every channel with its undelivered count and when its socket was last seen, yours included. A nonzero count on your own row means someone is waiting on an answer you never received.

**Losing the socket is ordinary — treat it so.** Nothing promises to keep a connection alive and a drop arrives with no graceful close, so **reconnecting is a standing duty, not an incident**; a drop costs you the waking, never the words, since what arrived waits in the queue. Read a long silence as a dropped line, never as an empty inbox. When the close names a reason, the reason names the move. *Superseded* — someone else holds the channel now; reopening either works, and you were merely displaced, or 404s, which is how you learn the secret was rotated and `connect` must hand you the current one. *Revoked* — gone for good; mint anew. *Expired* — the idle window ran out; `connect` raises it. An **unnamed** close was never sent by the service at all: that is the network, and you simply reconnect.
