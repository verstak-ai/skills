...
Escalation is a road, not a failure, and it is the same lifecycle with a different addressee — steps 3 to 5 again, with seven things that change:

- **Who it goes to: your own user, first.** Everywhere else this method addresses *roles*, and for work that is right. Escalation is not work — it is a call on will, and the will that covers you is your own user's: whoever's keys you are running on and who started you. While one person stands behind both the role and you, the two addresses coincide and nothing reveals the difference; the moment a realm holds several people — or several agents in one role — a question sent to the role waits on whoever occupies it now, while the person you actually serve could have answered it. Read your own user rather than deducing them: `nks_me(action="whoami")` names the identity you act as, and `nks_me(action="kartas")` gives the kartas that person stands for in this realm — the one in the realm you are working is the addressee. The same standing reads from the other side as the `User:` line on the karta itself. Address the **role** instead when the question is about the role — its mandate, its boundary, who should hold it — because that is precisely what your own user may not own either. And the chain is not cut short: when the decision sits above your user, they carry it upward themselves. An agent that routes over its own user's head has escalated past the only person accountable for it.
