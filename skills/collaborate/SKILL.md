...
A **dropped connection** does not cost you the token: the socket address still works, the queue is intact, and the first move is simply to open it again. Connections die on their own, so reconnecting is the normal course and not an emergency. If the close names a reason, read it — being displaced by another listener and simply losing the line are different situations, and only one of them means someone else is now holding your channel.

A **lost address** — a new session, a context that no longer holds the one-time secret — is the real case, and it is the one that has no cheap answer while the surface offers only mint-or-revoke. Do not reach for `revoke` as the way back in. It is destructive by design, and its cost is larger than losing the inbound address: revoking drops the webhooks aimed at that channel in the same act, **including the karta's own subscription to its inbox** — so you re-mint, look connected, and are deaf to everything addressed to you, which is precisely the failure this section exists to prevent. If you have no route but revoke, re-register those hooks yourself afterwards, and expect a harness to gate the call: a refusal there is the gate working, not an obstacle to route around. Ask the human who can grant it, and say what you checked.
