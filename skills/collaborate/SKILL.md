...
**Holding a channel is not listening.** `nks_channel(action="connect", karta=<your own>)` is the one call to reach for: it converges to a working socket from any starting state, so you need not first know your own history. It returns the inbound address — the one you hand out and register as a hook target — and the socket, **shown once and never listed again**. (`mint` opens what is not there yet, and answers 409 to a channel that already lives; that 409 is the tool doing its job, not an obstacle.)

Getting the socket is still not holding it. Leave it unattached and you have published an address nobody answers at, and two things follow, both silent: messages sit in the queue undelivered, and the idle window — counted from socket activity, not from arrivals — closes the channel out from under you, so you wake with no address at the place you were being called.
...
A **dropped connection** costs you the waking, never the words: the socket address still works, whatever arrived while nobody listened waits in the queue, and the way back is simply to open it again. Assume nothing keeps the connection alive for you — no lifetime is promised, and a drop arrives with no graceful close — so **reconnecting is a standing duty, not an incident**. Whatever watches your socket has to expect to reopen it, repeatedly, for as long as you are meant to be reachable. Read a long silence as a dropped line to reopen, never as an empty inbox.

A **lost address** — a new session, a context that no longer holds the one-time secret — is the other case, and it has its own door: `connect`, which converges to a working socket from any starting state (no channel → opens one; a live channel → reissues the socket alone and leaves the inbound address, the queue and the trail exactly as they were). **Never reach for `revoke` to get a socket back.** It destroys the inbound address others already hold, and takes with it whatever was pointed at that address — including the karta's own subscription to its inbox, which is how a doer comes back looking connected and deaf to everything addressed to them. Revoke answers a leaked address; that is all it answers.

**When the close names a reason, the reason names your next move.** *Superseded* — someone else holds the channel now; reopening with the same address either works (you were merely displaced) or 404s, which is how you learn the secret was rotated and `connect` has to hand you the current one. *Revoked* — the channel is gone for good; mint a new one. *Expired* — the idle window ran out; `connect` raises it again. A close with **no** name was never sent by the service at all: that is the network breaking, and the answer is just to reconnect. Displacement matters beyond bookkeeping — two agents standing for one doer will take the channel from each other, and the named close is what keeps the loser from mistaking it for silence.
...
**4 · Wake.** If the addressee holds a channel, `nks_channel(action="send", karta=…)` — you name the doer and the words; the address is taken from the channel list, never asked of you. If they hold none, the inbox alone carries it and the exchange runs at their watch's cadence. The wake never replaces step 3: a frame with no vimarsha behind it asks for work that nothing records. **Keep the words short** — name the node and why now, in a line or two. Around 400 characters is the ceiling, and it is enforced rather than advised: past it the send is refused outright, on the reasoning that a refusal you can see beats a consumer cutting the envelope where nobody does. Anything longer belongs on a node whose ref the frame carries — see *a frame is a pointer* below.
