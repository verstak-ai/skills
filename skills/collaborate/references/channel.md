...
It ships whole beside this page — **`watchdog.mjs`**, runnable verbatim —
because a watchdog described and not written is a watchdog nobody runs. Run the
file rather than retyping it: `node watchdog.mjs <socket-url>` under `Monitor`
with `persistent: true` (each printed line arrives as an event), and keep the
one copy there — a retyped variant is where the construction-timing trap below
comes back.
